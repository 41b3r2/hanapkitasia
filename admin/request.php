			...
			<div class="card-body pt-0">
				...
				<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_report_subject_list_table">
					...
					<tbody class="fw-bold text-gray-600">
						<?php
							$sql = "SELECT r.*,(SELECT f_name FROM residence WHERE id=r.residence_id) AS f_name,(SELECT m_name FROM residence WHERE id=r.residence_id) AS m_name,(SELECT l_name FROM residence WHERE id=r.residence_id) AS l_name,(SELECT cp_1 FROM residence WHERE id=r.residence_id) AS cp_1 FROM request r ORDER BY id DESC";
								    	...
								    	if($row['status']=="pending"){
								    		?>
									    		<td class="text-end text-primary">
													<a href="?page=request&verify=<?php echo$row['id']?>&number=<?php echo$row['cp_1']?>&type=<?php echo$row['type']?>" class="px-3 text-hover-info" data-toggle="tooltip" data-placement="top" title="Edit" >
														<i class="fa fa-check text-primary" aria-hidden="true"></i> Approve
													</a> |
													<a href="?page=request&declined=<?php echo$row['id']?>&number=<?php echo$row['cp_1']?>&type=<?php echo$row['type']?>" class="px-3 text-hover-info text-danger" data-toggle="tooltip" data-placement="top" title="Edit" >
														<i class="fa fa-trash text-danger" aria-hidden="true"></i> Declined
													</a>
												</td>
								    		<?php
										    ...
										    <td class="text-end text-success">
												<i class="fa fa-check text-success" aria-hidden="true"></i> Verified | <a href="print/?id=<?php echo$row['id']?>" target="_blank" class="text-hover-info text-primary"><i class="fa fa-file text-primary" aria-hidden="true"></i> Print</a>
											</td>
										    <?php
								    	}
